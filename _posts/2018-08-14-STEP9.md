...
4. **테스트코드 작성법**
   ...
   프로젝트에서 함수를 천개 만들었다고 가정해보죠. 점점 규모가 커질수록 사람이 일일히 해당 함수를 하나씩 테스트할 수 없다.즉, 어떤 수정이 일어났을 때(소프트웨어 업데이트, 새로운 버전 배포 등) 이전코드에 영향을 주지 않는지, 에러가 나진 않는지, **<u>최종적으로 정상적인 배포를 위하여</u>** 사람이 아닌 소프트웨어를 통해 테스트를 하기 위해서 테스트코드를 작성하는 것입니다.
