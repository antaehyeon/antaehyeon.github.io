...
1. 너무 이쁘게 남겨서 그런데, 클라이언트 개발하면서 실제로 로그를 찍는건 잘 안해요. 그 로그가 클라이언트 브라우저 노출되는거라 불필요하죠. 서버로 ajax로 보낼 수는 있고요.

   디버그용도로 찍고 있는걸로 알고 있을게요.

   `console.log("Success Load - Model Constructor");`

2. 좀더 범용적으로 하려면 setPropertyToItemNode 와 같은식도 가능하겠네요. setPropertyToItemNode(node, property='high-light')
